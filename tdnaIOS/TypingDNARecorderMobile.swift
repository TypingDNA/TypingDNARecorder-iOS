//
//  TypingDNARecorderMobile.swift iOS version
//  tdnaIOS
//
...
/**
 * Instantiate TypingDNARecorderMobile class in your project and make sure you also add UIViewExtention to your project
 */

// DO NOT MODIFY
open class TypingDNARecorderMobile {
    ...
    public init() {
        if (!TypingDNARecorderMobile.initialized) {
            TypingDNARecorderMobile.initialized = TypingDNARecorderMobile.initialize();
        }
    }
...
extension UIWindow {
    ...
    override open func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if (!TypingDNARecorderMobile.KIOSdidSetup) {
        ...
        if (event != nil) {
            ...
            if (time != TypingDNARecorderMobile.KIOSlastTimeStamp) {
                TypingDNARecorderMobile.KIOSkeyPressed(point);
                UIW_KIOSsetResponder();
                TypingDNARecorderMobile.KIOSlastTimeStamp = time;
            }
        }
    ...
    @objc func UIW_KIOSkeyReleased(_ sender: UITextInput) {
        ...
        TypingDNARecorderMobile.KIOSkeyReleased(targetHash);
    }
    
    @objc func UIW_KIOSsetResponder() {
        if let x = UIResponder.current as? UITextField {
            if x != TypingDNARecorderMobile.KIOScurrentResponder {
                TypingDNARecorderMobile.KIOScurrentResponder = x;
                TypingDNARecorderMobile.KIOSlastText = String(describing: x.text!);
                let actions = TypingDNARecorderMobile.KIOScurrentResponder.actions(forTarget: nil, forControlEvent: .editingChanged);
                if(actions == nil || !actions!.contains("UIW_KIOSkeyReleased:")) {
                    TypingDNARecorderMobile.KIOScurrentResponder.addTarget(self, action: #selector(UIW_KIOSkeyReleased(_:)), for: .editingChanged);
                }
            }
        }
    }
